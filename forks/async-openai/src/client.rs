...
use crate::{
    ...
    traits::AsyncTryFrom,
    types::stream::{SseEvent, SseResponse},
    RequestOptions,
};
...
impl<C: Config> Client<C> {
    ...
    async fn execute_stream_with_done<O>(
        &self,
        request_factory: HttpRequestFactory,
    ) -> Result<SseResponse<O>, OpenAIError>
    where
        O: DeserializeOwned + crate::traits::MaybeSend + 'static,
    {
        let response = self.execute_response(request_factory).await?;
        Ok(stream_with_done(response).await)
    }
    ...
    pub(crate) async fn post_stream_with_done<I, O>(
        &self,
        path: &str,
        request: I,
        request_options: &RequestOptions,
        ...
        O: DeserializeOwned + crate::traits::MaybeSend + 'static,
    {
        ...
        self.execute_stream_with_done(request_factory).await
    }
...
pub(crate) async fn stream_with_done<O>(response: Response) -> SseResponse<O>
where
    O: DeserializeOwned + crate::traits::MaybeSend + 'static,
{
    stream_mapped_raw_events_with_done(response, |event| {
        serde_json::from_str::<O>(&event.data)
            .map_err(|error| map_deserialization_error(error, event.data.as_bytes()))
    })
    .await
...
{
    let stream = stream_mapped_raw_events_with_done(response, event_mapper).await;

    Box::pin(stream.filter_map(|event| async move {
        match event {
            Ok(SseEvent::Event(event)) => Some(Ok(event)),
...
#[cfg(target_family = "wasm")]
pub(crate) async fn stream_mapped_raw_events_with_done<O>(
    response: Response,
    event_mapper: impl Fn(eventsource_stream::Event) -> Result<O, OpenAIError> + 'static,
) -> SseResponse<O>
where
    O: DeserializeOwned + 'static,
{
    ...
    Box::pin(futures::stream::unfold(
        ...
        |(mut event_stream, event_mapper, done)| async move {
            if done {
                return None;
            }
            ...
            loop {
                ...
                if event.data == "[DONE]" {
                    return Some((Ok(SseEvent::Done), (event_stream, event_mapper, true)));
                }
                ...
                let response = event_mapper(event).map(SseEvent::Event);
                return Some((response, (event_stream, event_mapper, false)));
            }
        },
    ))
}
...
mod tests {
    ...
    use crate::{
        ...
        retry::SimpleRetryPolicy, traits::AsyncTryFrom, types::stream::SseEvent, RequestOptions,
    };
    ...
    #[tokio::test]
    async fn stream_with_done_preserves_the_terminal_sentinel() {
        let service = ServiceBuilder::new().service(service_fn(
            move |_factory: HttpRequestFactory| async move {
                Ok::<reqwest::Response, OpenAIError>(
                    HttpResponse::builder()
                        .status(200)
                        .header("content-type", "text/event-stream")
        ...
        .with_http_service(service);
        let mut stream = client
            .post_stream_with_done::<_, serde_json::Value>(
                "/responses",
                json!({ "stream": true }),
                &RequestOptions::new(),
            )
        ...
        assert_eq!(
            ...
            SseEvent::Event(json!({ "ok": true }))
        );
        assert_eq!(stream.next().await.unwrap().unwrap(), SseEvent::Done);
        assert!(stream.next().await.is_none());
    }
