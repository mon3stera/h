use crate::{
    ...
    types::responses::{
        ...
        ResponseItemList, ResponseStreamEvent, TokenCountsBody, TokenCountsResource,
    },
    Client, RequestOptions,
};

use crate::types::responses::ResponseStream;
use crate::types::stream::SseResponse;
...
impl<'c, C: Config> Responses<'c, C> {
    ...
    /// Creates a streaming model response while preserving the terminal
    /// `data: [DONE]` SSE sentinel as a stream item.
    pub async fn create_stream_with_done(
        &self,
        mut request: CreateResponse,
    ) -> Result<SseResponse<ResponseStreamEvent>, OpenAIError> {
        request.stream = Some(true);

        self.client
            .post_stream_with_done("/responses", request, &self.request_options)
            .await
    }

    #[cfg(feature = "byot")]
    pub async fn create_stream_with_done_byot<T, R>(
        &self,
        request: T,
    ) -> Result<SseResponse<R>, OpenAIError>
    where
        T: serde::Serialize,
        R: serde::de::DeserializeOwned + crate::traits::MaybeSend + 'static,
    {
        self.client
            .post_stream_with_done("/responses", request, &self.request_options)
            .await
